...
1) go run main.go из \arithmetic-expression-calculator\cmd\agent
2) go run main.go из \arithmetic-expression-calculator\cmd\orchestrator
...
Cтрока с выражением не поддерживает деление на ноль и унарный минус.
Для дробных чисел используется точка: 6.6
...
```
curl -X POST -H "Content-Type: application/json" --location 'http://localhost:8081/internal/task' --data '{"id": "1-1","result": 2}'
```
